...
From `03_check_inputs.py` on `basic_test`:
...
> **Run `03_check_inputs.py` against the real TC table before designing the
...
0. **How many resources in the real TC table actually split?** Run
   `03_check_inputs.py` against it. This is a five-minute empirical question
