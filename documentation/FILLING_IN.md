...
**All three cases** have a **`TCs_improved`** sheet beside `TCs`, seeded
2026-09-03 as an exact copy — 24 rows for wiring, 52 for boards, 632 for car
composition — and an improvement window in the `source` sheet:

    improvement_start   2030
    improvement_end     2060

**As seeded they change nothing.** Every value equals its current one, so a
case ramps from a number to itself and every year is identical -- verified row
for row against the run before the sheet existed, to zero difference (517 rows
wiring, 1,650 boards). Car composition could not be run that way, because its
upstream export is waiting to be rebuilt, so it was checked on the tables
instead: all 11 years equal the current table exactly, and all 3,894 groups
(354 resources x 11 years) close to 1 exactly. They are places to type into,
not scenarios.

**Car composition has one extra hazard.** Its `TCs` sheet is GENERATED by
`tools/make_carcomposition_tcs.py`, which knows nothing about `TCs_improved`,
so a rebuild leaves the improved sheet describing the table that used to be
there. The run refuses a mismatch rather than ramping towards the wrong
coefficient, and the tool now says so at the moment it rebuilds -- but if you
regenerate that table, re-seed the improved sheet from it.
