...
| [DESIGN_tc_table.md](DESIGN_tc_table.md) | How to build the TC table so that sum-to-1 holds by construction. Proposal, with a worked example in `data_folder/reference/template`. **Both cases now have tables built this way** — what they lack is measured numbers, not structure. See CASES.md for the rules as implemented. |
| [DESIGN_04_01_carcomposition.md](DESIGN_04_01_carcomposition.md) | What it would take to read stage 04_01 (car composition) as well as 04_02. Effort estimate, layer mapping, and a made-up TC table on the real component and material names. **Built 2026-08-21** — see `data_folder/carcomposition_mockup`, and the document's own record of what building it proved the estimate had wrong. |
